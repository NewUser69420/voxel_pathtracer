        ...
        if node.voxel.id != 0 {
            ...
            for (var i = 0u; i < emitter_num; i++) {
                ...
                if dist_to_light < light.range {
                    let dir = normalize(light.position - photon);
                    var ray = AabbRay(at_length(AabbRay(photon, dir, vec3<f32>()), 1.0), dir, vec3<f32>());
                    ray.inv_direction = vec3<f32>(1.0/ray.direction.x, 1.0/ray.direction.y, 1.0/ray.direction.z);

                    var result = cast_ray(ray, light.range);

                    //lighting
                    if result.dist > dist_to_light - light.radius {
                        let range_mod = map_range(
                        0.0, light.range,
                        0.0, 1.0,
                        (dist_to_light + (rndm2 * LIGHTDIFFUSION)) * (1 / light.falloff),
                        );

                        let light_mod = 1.0 - range_mod;
                        intensity = light_mod * light.strength;
                        light_color = max(light_color, light.color * intensity);
                    }                                                        
                }                                
            }
...
fn cast_ray(r: AabbRay, range: f32) -> RayResult {
    ...
    while (length < range && steps < MAXSTEP) {
        ...
        if node.voxel.id != 0 {
            return RayResult(length, vec3<f32>(node.voxel.color[0], node.voxel.color[1], node.voxel.color[2]));
        }
    ...
    return RayResult(length, vec3<f32>(0.0, 0.0, 0.0));
}
